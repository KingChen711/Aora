...
export async function uploadFile(file, type) {
  ...
  const { mimeType, fileSize, uri, fileName } = file
  const asset = { type: mimeType, name: uri.substring(uri.lastIndexOf('/') + 1, uri.length), size: fileSize, uri }
...
export async function createVideoPost(form) {
  try {
    ...
    console.log('Error: createVideoPost')
    throw new Error(error)
  }
}
